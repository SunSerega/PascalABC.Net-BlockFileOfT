...
///Тип-альтернативу стандартному file of T
...
type
  ...
  BlockFileOf<T>=class(BlockFileBase)
    ...
    ///Привязывать можно и к не существующим файлам, при открытии определёнными способами (как Rewrite) новый файл будет создан
    ...
    ///Чтоб получить переменную типа FileMode - пишите System.IO.FileMode.<способ_открытия_файла>
    ...
    ///Читает массив из count элементов одним блоком, начиная с элемента файла start_elm
    ...
    ///После завершения чтения курсор окажется на элементе #(start_elm+count)
    ...
    ///Каждый блок хранит столько элементов, чтобы не превышать объём в 4096 байт (4КБ)
    ...
    ///Возвращает ленивую последовательность из блоков-массивов с элементами типа T
    ///Каждый блок хранит столько элементов - чтобы не превышать объём в последовательность байт
    ...
    ///После чтения i элементов последовательности - позиция в файле будет передвигаться на элемент #i
