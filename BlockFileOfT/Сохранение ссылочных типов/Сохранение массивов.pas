...
type
  ...
  //  -Поменять название (число означающее длину и название типа элементов массива)
  //  -Поменять значение константы MaxLength на нужную длину
  //  -Поменять значение константы TSize на объём памяти, необходимый под каждый элемент массива
  //  -Поменять тип Arr15Int в операторах explicit на новый (хотя, компилятор и не даст его оставить Arr15Int)
  //  -Поменять тип array of integer в операторах explicit на array of <Новый тип элементов>
