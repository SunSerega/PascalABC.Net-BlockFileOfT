...
type
  ...
  r3 = record
    ...
    // Но поэтому же надо относится с осторожностью к такой возможности
    // И помнить у какого типа какой размер, чтоб наложение полей не получилось там где не надо
...
begin
  ...
  // А вот так делать не надо. Сейчас курсор находится в середите второго элемента
  // Обычно это вызовет неопределённое поведение и заполнит поля полученной записи мусором
  // Но этот случай простой, поэтому точно известно что прочитает половину второй + половину третьей записи и выведет (4,5)
  Writeln(f.Read);
  ...
  f.Pos := 0;
  f.PosByte += 1; // сдвигаем курсор на 1 байт (вся запись f.TSize байт, что, в данном случае, 2)
  ...
  // Это так же хорошая причина не пользоваться бездумно BaseStream - потому что на разных компьютерах один и тот же код может работать по-разному
  Writeln(f2.TSize);
